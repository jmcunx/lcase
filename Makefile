...
LFLAGS=$(WALL) $(LIBJLIB2) -o lcase $(LIBS)

ALL_OBJ=lcase$(OBJ) lcase_h$(OBJ) lcase_u$(OBJ)
ALL_C=lcase.c lcase_h.c lcase_u.c
ALL_H=lcase.h

#
# do the work
#
all:	ckenv tags $(ALL_OBJ) lcase.1$(GZ)
...
install:	all
	...
	-$(CHMOD) 755 $(PRODMAN)
	-$(STRIP) lcase
	$(CP) lcase $(PRODUCTION)/lcase
	$(CHMOD) 755 $(PRODUCTION)/lcase
	$(CP) lcase.1$(GZ) $(PRODMAN)/lcase.1$(GZ)
	$(CHMOD) 644 $(PRODMAN)/lcase.1$(GZ)

uninstall:	all
	-$(RM) $(PRODUCTION)/lcase
	-$(RM) $(PRODMAN)/lcase.1.gz
	-$(RM) $(PRODMAN)/lcase.1
